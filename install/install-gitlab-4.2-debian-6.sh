...
#
# This installer is mainly based on https://github.com/gitlabhq/gitlabhq/blob/4-2-stable/doc/install/installation.md
# 
...
sudo apt-get install -y nano mysql-server mysql-client libmysqlclient-dev wget curl gcc checkinstall libxml2-dev libxslt1-dev libcurl4-openssl-dev libreadline6-dev libc6-dev libssl-dev libmysql++-dev make build-essential zlib1g-dev libicu-dev openssh-server git-core python-dev python-pip libyaml-dev libpq-dev libffi-dev libgdbm-dev libreadline-dev libncurses5-dev libxslt-dev nginx

# needs to be ver 2.0+
apt-get -t squeeze-backports install redis-server

# make sure we can use pythyn by python2 binary
...
# setup DB
echo -e "Please provide the MySQL root pw"
read mysqlrootpw

echo -e "Please provide a pw for gitlab db user"
read gitlabpw

# login as root and create user and database 
mysql -u root -p${mysqlrootpw} -e "CREATE USER 'gitlab'@'localhost' IDENTIFIED BY '$gitlabpw';"
mysql -u root -p${mysqlrootpw} -e "CREATE DATABASE IF NOT EXISTS \`gitlabhq_production\` DEFAULT CHARACTER SET \`utf8\` COLLATE \`utf8_unicode_ci\`;"
mysql -u root -p${mysqlrootpw} -e "GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, INDEX, ALTER ON \`gitlabhq_production\`.* TO 'gitlab'@'localhost';"
...
sudo -u gitlab -H git config --global user.email "${mailsender}"
...
sed -i "/  password: / c \ \ password: \"${gitlabpw}\"" /home/gitlab/gitlab/config/database.yml
sed -i "/  username: / c \ \ username: \"gitlab\"" /home/gitlab/gitlab/config/database.yml
...
# setup GitLab hooks
sudo cp -a /home/gitlab/gitlab/lib/hooks/post-receive /home/git/.gitolite/hooks/common/post-receive
sudo chown git:git /home/git/.gitolite/hooks/common/post-receive
